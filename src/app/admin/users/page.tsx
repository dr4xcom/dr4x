  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // نحاول أولاً: order(created_at)
        // لو فشل بسبب عدم وجود العمود أو RLS/permissions، نسوي fallback لطلب أبسط
        const buildBase = () =>
          supabase
            .from("profiles")
        ...
        if (!attempt1.error) {
          ...
          setTotalCount(typeof attempt1.count === "number" ? attempt1.count : null);
          setLoading(false);
          return;
        }

        // محاولة 2 (fallback): بدون order(created_at) لأن بعض المشاريع لا تحتوي created_at أو RLS تمنع
        // نستخدم order(id) كترتيب "مستقر"
        ...
        setTotalCount(typeof attempt2Res.count === "number" ? attempt2Res.count : null);
        setLoading(false);
      } catch (e: any) {
        if (!alive) return;
        setErr(
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          ...
          <div className="text-sm text-slate-300">
            عرض المستخدمين من جدول profiles + بحث/فلترة (بدون تعديل DB/RLS).
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-950/40 overflow-hidden">
                ...
                <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-xs text-slate-500 truncate">ID: {p.id}</span>
                    </div>
        ...
        <div className="px-4 py-3 border-t border-slate-800 flex items-center justify-between">
          <div className="text-xs text-slate-400">
            ملاحظة: هذه الصفحة للعرض فقط. لو احتجنا تعديل profiles بشكل آمن، سنثبت أولًا أن UPDATE ممنوع عبر RLS،
            ثم نستخدم RPC بأقل تعديل ممكن.
          </div>
