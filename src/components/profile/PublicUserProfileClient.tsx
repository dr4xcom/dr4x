...
export default function PublicUserProfileClient() {
      ...
      try {
        ...
        if (uid) {
          try {
            ...
            if (notifErr) {
              console.error("user_list_notifications error (badge)", notifErr);
  ...
  // ✅ زر العيادة يظهر فقط لصاحب الحساب إذا كان is_doctor = true
        ...
        {!profile ? null : (
          <>
            ...
            <div className="rounded-[32px] border border-slate-800 bg-slate-950/80 px-6 py-5 mt-4">
              <div className="text-base sm:text-lg font-semibold">نبذة عني</div>
